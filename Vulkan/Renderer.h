...
private:
	...
	void createGraphicsPipeline()
	{
		...
		//This is the function name to invoke in the shader code, 
		//this makes it possible to combine multiple programs into a single source file.
		...
		//Use VK_POLYGON_MODE_LINE for lines or _POINT for points
	...
	VkSurfaceFormatKHR chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats)
	{
		...
		{
			std::cout << "IDEAL SWAP CHAIN SURFACE FORMAT SUPPORTED" << std::endl;
	...
	void pickPhysicalDevice()
	{
		...
		if (deviceCount == 0)
		{
			...
			std::cout << "FOUND " << deviceCount << " DEVICE(S) WITH VULKAN SUPPORT" << std::endl;
		...
		for (const auto& device : devices)
		{
			if (isDeviceSuitable(device))
			{
				...
				std::cout << "FOUND GPU SUITABLE FOR USE." << std::endl;
				break;
			}
		}
	...
	bool isDeviceSuitable(VkPhysicalDevice device)
	{
		...
		//TO DO: Pick best GPU based on score.
